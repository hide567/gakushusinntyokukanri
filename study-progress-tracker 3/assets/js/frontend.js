/**
 * 学習進捗管理用のインタラクティブ機能（AJAX改善・デバッグ強化版）
 */
jQuery(document).ready(function($) {
    ...
    function debugLog(message, data = null) {
        if (DEBUG_MODE) {
            console.log('[SPT Debug]', message, data || '');
        }
    }
    
    debugLog('学習進捗管理システム初期化開始');
    ...
    $(document).off('click.spt').on('click.spt', '.progress-tracker-shortcode .chapter-accordion-header', function(e) {
        ...
        debugLog('章アコーディオンクリック', {chapter: chapterId, subject: subjectKey});
    ...
    $(document).off('click.spt-tab').on('click.spt-tab', '.progress-tracker-shortcode .progress-tab', function(e) {
        ...
        debugLog('タブ切り替え', {subject: subjectKey});
    ...
    $(document).off('change.spt-check1').on('change.spt-check1', '.item-check-level-1', function(e) {
        ...
        debugLog('理解レベルチェック変更', {checked: isChecked, container: $container.data()});
    ...
    $(document).off('change.spt-check2').on('change.spt-check2', '.item-check-level-2', function(e) {
        ...
        debugLog('習得レベルチェック変更', {checked: isChecked, container: $container.data()});
    ...
    function updateStatus($container, level) {
        ...
        debugLog('状態更新開始', {subject, chapter, section, item, level});
    ...
    function saveStatus($container, level) {
        ...
        if (!nonce) {
            ...
            showError('保存に必要なセキュリティ情報が不足しています。ページを再読み込みしてください。');
            return;
        }
        ...
        $.ajax({
                ...
                if (response && response.success) {
                    ...
                    if (response.data && typeof response.data.percent !== 'undefined') {
                        ...
                        debugLog('進捗データ更新完了', {
                            ...
                            chapter_completed: response.data.chapter_completed,
                            section_completed: response.data.section_completed
                        });
                    }
                } else {
                    const errorMessage = response?.data?.message || 'データの保存に失敗しました。';
                    console.error('保存エラー:', response?.data || 'Unknown error');
                    showError('更新に失敗しました: ' + errorMessage);
                    // 失敗時は元の状態に戻す
            ...
            error: function(xhr, status, error) {
                ...
                debugLog('Ajax保存エラー', {status, error, responseText: xhr.responseText});
                
                let errorMessage = '';
                if (status === 'timeout') {
                    errorMessage = '保存がタイムアウトしました。インターネット接続を確認してから再度お試しください。';
                } else if (status === 'error' && xhr.status === 0) {
                    errorMessage = 'インターネット接続に問題があります。接続を確認してから再度お試しください。';
                } else if (xhr.status === 403) {
                    errorMessage = 'セキュリティエラーが発生しました。ページを再読み込みしてから再度お試しください。';
                } else if (xhr.status === 500) {
                    errorMessage = 'サーバーエラーが発生しました。しばらく待ってから再度お試しください。';
                } else {
                    errorMessage = '通信エラーが発生しました。ページを再読み込みしてから再度お試しください。';
                }
    ...
    function showSavingIndicator(show) {
        ...
        if ($indicator.length === 0) {
            $indicator = $('<div class="saving-indicator" style="position: fixed; top: 20px; right: 20px; background-color: #0073aa; color: white; padding: 10px 20px; border-radius: 5px; box-shadow: 0 2px 5px rgba(0,0,0,0.2); z-index: 9999; display: none; font-size: 14px; font-weight: bold;"></div>');
            $('body').append($indicator);
        }
        
        if (show) {
            $indicator.text('保存中...').addClass('show').fadeIn(200);
        } else {
    ...
    function revertCheckboxState($container) {
        ...
        setTimeout(() => {
            if (confirm('エラーが発生しました。ページを再読み込みしますか？')) {
                ...
                // リロードしない場合は手動で状態を復元
                updateContainerStyle($container, 0);
    ...
    function resetProgress(subject) {
        if (!nonce) {
            showError('セキュリティエラーが発生しました。ページを再読み込みしてください。');
            return;
        }
        
        debugLog('進捗リセット開始', {subject});
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (response && response.success) {
                    ...
                    if (typeof window.showNotification === 'function') {
                        window.showNotification('進捗をリセットしました。', 'success');
                    } else {
                        alert('進捗をリセットしました。');
                    }
                } else {
                    const errorMessage = response?.data?.message || '不明なエラー';
                    showError('リセットに失敗しました: ' + errorMessage);
                }
            },
            error: function(xhr, status, error) {
                $(`.progress-subject[data-subject="${subject}"]`).css('opacity', 1);
                
                debugLog('進捗リセットエラー', {status, error});
                
                if (status === 'timeout') {
                    showError('リセット処理がタイムアウトしました。');
                } else {
                ...
                if (chapterId && subjectKey) {
                    try {
                        ...
                        if (savedState === 'expanded') {
                            $content.show();
                            $icon.text('-');
                            debugLog('章展開復元', {subject: subjectKey, chapter: chapterId});
                        } else if (savedState === 'collapsed') {
                            $content.hide();
                            $icon.text('+');
                            debugLog('章折りたたみ復元', {subject: subjectKey, chapter: chapterId});
                        }
    ...
    function checkConnection() {
        if (typeof navigator !== 'undefined' && !navigator.onLine) {
            showError('インターネット接続が必要です。接続を確認してください。');
            return false;
        }
        return true;
    }
    ...
    if (typeof window !== 'undefined') {
        ...
        $(window).on('offline.spt', function() {
            ...
            showError('インターネット接続が切断されました。変更は保存されない可能性があります。');
        });
    }
    ...
    if (DEBUG_MODE && typeof performance !== 'undefined') {
        $(window).on('load', function() {
            ...
            debugLog('ページ読み込み完了', {loadTime: loadTime + 'ms'});
        });
    }
    
    // デバッグ用コンソール出力
    debugLog('学習進捗管理システム（AJAX改善・デバッグ強化版）が初期化されました');
    
    // 初期化完了の通知
    $(document).trigger('spt-initialized', {
        version: '2.0',
        debug: DEBUG_MODE,
        features: ['ajax_improvement', 'debug_enhancement', 'progress_bar_styles']
    });
});