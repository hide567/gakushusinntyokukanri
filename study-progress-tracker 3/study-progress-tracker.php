 ...
 * Description: 行政書士試験などの資格試験学習の進捗を管理するプラグイン（進捗バー設定対応版）
 * Version: 2.0.0
 * Author: Your Name
...
class StudyProgressTracker {
    ...
    public function enqueue_scripts() {
        ...
        if (is_user_logged_in()) {
            ...
            wp_localize_script('spt-frontend', 'progress_tracker', array(
                ...
                'progress_bar_style' => $check_settings['progress_bar_style'],
                'progress_bar_animation' => $check_settings['progress_bar_animation']
            ));
        }
    }
    ...
    // Ajax: 項の完了状態を切り替え（デバッグ強化版）
    public function ajax_toggle_item_completion() {
        // セキュリティチェック
        if (!check_ajax_referer('progress_tracker_nonce', 'nonce', false)) {
            error_log('SPT Ajax Error: Nonce verification failed');
            wp_send_json_error(array('message' => __('セキュリティチェックに失敗しました。', 'study-progress-tracker')));
            return;
        }
        
        if (!is_user_logged_in()) {
            error_log('SPT Ajax Error: User not logged in');
            wp_send_json_error(array('message' => __('ログインが必要です。', 'study-progress-tracker')));
            return;
        }
        
        // パラメータの取得と検証
        $subject_key = isset($_POST['subject']) ? sanitize_key($_POST['subject']) : '';
        ...
        $check_level = isset($_POST['check_level']) ? intval($_POST['check_level']) : 0;
        
        error_log("SPT Ajax Request: {$subject_key}, Ch{$chapter_id}, Sec{$section_id}, Item{$item_id}, Level{$check_level}");
        
        // 必要なパラメータの確認
        if (empty($subject_key) || $chapter_id <= 0 || $section_id <= 0 || $item_id <= 0) {
            error_log('SPT Ajax Error: Missing required parameters');
            wp_send_json_error(array('message' => __('必要なパラメータが不足しています。', 'study-progress-tracker')));
            return;
        }
        ...
        if (!isset($subjects[$subject_key])) {
            error_log("SPT Ajax Error: Subject {$subject_key} not found");
            wp_send_json_error(array('message' => __('指定された科目が存在しません。', 'study-progress-tracker')));
            return;
        }
        ...
        // データを保存
        $saved = update_option('progress_tracker_progress', $progress_data);
        
        if (!$saved) {
            error_log('SPT Ajax Error: Failed to save progress data');
            wp_send_json_error(array('message' => __('データの保存に失敗しました。', 'study-progress-tracker')));
            return;
        }
        ...
        error_log("SPT Ajax Success: Progress saved, {$progress_data[$subject_key]['percent']}%");
    ...
    public function ajax_reset_progress() {
        // セキュリティチェック
        if (!check_ajax_referer('progress_tracker_nonce', 'nonce', false)) {
            error_log('SPT Reset Error: Nonce verification failed');
            wp_send_json_error(array('message' => __('セキュリティチェックに失敗しました。', 'study-progress-tracker')));
            return;
        }
        
        if (!is_user_logged_in()) {
            error_log('SPT Reset Error: User not logged in');
            wp_send_json_error(array('message' => __('ログインが必要です。', 'study-progress-tracker')));
            return;
        }
        ...
        if (empty($subject_key)) {
            error_log('SPT Reset Error: No subject specified');
            wp_send_json_error(array('message' => __('科目が指定されていません。', 'study-progress-tracker')));
            return;
        }
        
        error_log("SPT Reset: Resetting progress for {$subject_key}");
        ...
        if (isset($progress_data[$subject_key])) {
            ...
            if (!$saved) {
                error_log("SPT Reset Error: Failed to save reset data for {$subject_key}");
                wp_send_json_error(array('message' => __('リセットに失敗しました。', 'study-progress-tracker')));
                return;
            }
            
            error_log("SPT Reset Success: {$subject_key} progress reset");
        }
        
        wp_send_json_success(array('message' => __('進捗をリセットしました。', 'study-progress-tracker')));
    }
    ...
    private function recalculate_progress($subject_key, &$progress_data, $chapter_structure) {
        ...
        error_log("SPT Progress Calculation: {$subject_key} = {$completed_count}/{$total_items} = {$percent}%");
    }
    ...
    private function get_chapter_status($subject_key, $chapter_id, $progress_data, $chapter_structure) {
        ...
        if (isset($chapter_structure[$subject_key]['chapters'][$chapter_id])) {
            ...
            if (isset($chapter_info['section_data'])) {
                $total_sections = count($chapter_info['section_data']);
    ...
    public function activate() {
        ...
        // ログ出力
        error_log('SPT: Plugin activated successfully');
    }
    ...
    public function deactivate() {
        ...
        // ログ出力
        error_log('SPT: Plugin deactivated');
    }
}
