        ...
        foreach ($structure['chapters'] as $chapter) {
            ...
            foreach ($chapter['sections'] as $section) {
                ...
                foreach ($section['items'] as $item) {
                    $total_items++;
                    $key = $chapter['chapter_number'] . '-' . $section['section_number'] . '-' . $item['item_number'];
...
foreach ($subjects as $subject) {
    ...
    foreach ($progress as $item) {
        ...
        $progress_data[$subject->subject_key][$key] = array(
        ...
        <?php if ($atts['mode'] === 'summary'): ?>
            ...
            <div class="spm-mode-summary">
                <?php foreach ($subjects as $subject): ?>
                    ...
                    $subject_progress = isset($progress_data[$subject->subject_key]) ? $progress_data[$subject->subject_key] : array();
                    $overall_progress = spm_calculate_subject_progress($subject->subject_key, $subject_structure, $subject_progress);
                    ?>
                            ...
                            foreach ($subject_structure['chapters'] as $chapter) {
                                ...
                                foreach ($chapter['sections'] as $section) {
                                    ...
                                    foreach ($section['items'] as $item) {
                                        ...
                                        if (isset($subject_progress[$key])) {
                                            if ($subject_progress[$key]['understanding']) $understood_items++;
                                            if ($subject_progress[$key]['mastery']) $mastered_items++;
                                        }
                                    }
                                }
                            }
                                        ...
                                        foreach ($chapter['sections'] as $section) {
                                            if (isset($section['items'])) {
                                                foreach ($section['items'] as $item) {
                                                    ...
                                                    if (isset($subject_progress[$key]) && $subject_progress[$key]['mastery']) {
                                                        $chapter_mastered++;
                                                    }
                                                }
                                            }
                                                            ...
                                                            <div class="spm-items-grid">
                                                                ...
                                                                <?php else: ?>
                                                                    <?php foreach ($section['items'] as $item): ?>
                                                                        <?php
                                                                        $item_key = $chapter['chapter_number'] . '-' . $section['section_number'] . '-' . $item['item_number'];
                                                                        $item_progress = isset($subject_progress[$item_key]) ? $subject_progress[$item_key] : array('understanding' => false, 'mastery' => false);
                                                                        ?>
        ...
        saveProgress: function() {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    showSaveIndicator('error', '保存に失敗しました: ' + (data.message || ''));
