...
jQuery(document).ready(function($) {
    ...
    /**
     * 進捗管理タブでの章アコーディオン（イベント委譲で重複回避）
     */
    $(document).on('click.spt-admin', '.chapter-accordion-header', function(e) {
        e.preventDefault();
        e.stopPropagation();
     ...
     * 進捗管理タブでのタブ切り替え（重複回避）
     */
    $(document).on('click.spt-admin', '.progress-tab', function(e) {
        e.preventDefault();
        e.stopPropagation();
    ...
    function createSectionDetailsHtml(subject, chapterNum, sectionNum) {
        return '<div class="section-details" id="section-' + subject + '-' + chapterNum + '-' + sectionNum + '-details" style="display: none;">' +
        ...
        if ($('.progress-tab').length > 0) {
            try {
                ...
                if (savedTab && $('.progress-tab[data-subject="' + savedTab + '"]').length) {
                    $('.progress-tab[data-subject="' + savedTab + '"]').trigger('click');
                } else {
    ...
    console.log('学習進捗管理システム管理画面（修正版）が初期化されました');
});