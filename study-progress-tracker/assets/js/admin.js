/**
 * 学習進捗管理の管理画面用JavaScript（リアルタイム保存対応版）
 *
...
jQuery(document).ready(function($) {
            ...
            success: function(response) {
                ...
                if (response && response.success) {
                    ...
                    if (response.data && typeof response.data.percent !== 'undefined') {
                        ...
                        var subjectKey = $subjectProgress.data('subject');
                    ...
                    alert('更新に失敗しました: ' + (response?.data?.message || 'エラーが発生しました'));
    ...
    function showAdminSavingIndicator(show) {
        ...
        if ($indicator.length === 0) {
            $indicator = $('<div class="admin-saving-indicator" style="position: fixed; top: 32px; right: 20px; background-color: #0073aa; color: white; padding: 8px 15px; border-radius: 3px; z-index: 99999; display: none; font-size: 13px;">保存中...</div>');
    ...
    function revertCheckboxStateAdmin($itemRow, $firstCheck, $secondCheck) {
        var confirmReload = confirm('エラーが発生しました。ページを再読み込みしますか？');
        if (confirmReload) {
            location.reload();
        }
    }
    ...
    function getAdminNonce() {
        // ページ内のnonceを探す
        var nonce = $('input[name="_wpnonce"]').first().val();
        if (!nonce) {
            // フォーム内のnonceを探す
            nonce = $('form input[name="_wpnonce"]').first().val();
        }
        return nonce || '';
    }
    ...
    console.log('学習進捗管理システム管理画面（リアルタイム保存対応版）が初期化されました');
});