...
class SPT_Admin {
    ...
    private function handle_post_requests(&$subjects, &$chapter_structure, &$progress_data, &$progress_settings, &$custom_subjects) {
        // 科目と章の設定を保存
        if (isset($_POST['save_structure']) && check_admin_referer('spt_save_structure')) {
            $this->save_chapter_structure($subjects, $chapter_structure);
        }
        
        // 進捗データを保存
        if (isset($_POST['save_progress']) && check_admin_referer('spt_save_progress')) {
            $this->save_progress_data($subjects, $chapter_structure, $progress_data);
        }
        
        // 進捗データのリセット
        if (isset($_POST['reset_progress']) && isset($_POST['confirm_reset']) && $_POST['confirm_reset'] == '1' && check_admin_referer('spt_reset_progress')) {
            $this->reset_progress($progress_data);
        }
        
        // カスタム科目の追加
        if (isset($_POST['add_subject']) && check_admin_referer('spt_add_subject')) {
            $this->add_custom_subject($subjects, $custom_subjects, $chapter_structure, $progress_data);
        }
        
        // 科目の編集
        if (isset($_POST['edit_subject']) && check_admin_referer('spt_edit_subject')) {
            $this->edit_subject($subjects, $custom_subjects, $chapter_structure);
        }
        
        // 科目の削除
        if (isset($_POST['delete_subject']) && check_admin_referer('spt_delete_subject')) {
            $this->delete_subject($subjects, $custom_subjects, $chapter_structure, $progress_data);
        }
        
        // 進捗チェック設定の保存
        if (isset($_POST['save_check_settings']) && check_admin_referer('spt_save_check_settings')) {
