...
jQuery(document).ready(function($) {
    // 色設定パネルの表示/非表示
    $('.color-settings-toggle').on('click', function(e) {
        e.preventDefault();
        e.stopPropagation();
    ...
    // 進捗管理タブでのタブ切り替え（イベントバブリング防止）
    $('.progress-tab').on('click', function(e) {
        e.preventDefault();
        e.stopPropagation();
        
        var subject = $(this).data('subject');
        
        // 既にアクティブなタブがクリックされた場合は何もしない
        ...
        console.log('タブ切り替え: ' + subject); // デバッグ用
    });
    
    // 章アコーディオン（イベントバブリング防止とセレクター特定）
    $(document).on('click', '.chapter-accordion-header', function(e) {
        e.preventDefault();
        e.stopPropagation();
        ...
        var $header = $(this);
        var $content = $header.siblings('.chapter-accordion-content');
        var $icon = $header.find('.chapter-toggle-icon');
        ...
        console.log('章アコーディオン操作'); // デバッグ用
    });
    
    // 理解チェックボックスの処理
    $(document).on('change', 'input[type="checkbox"]:not([name*="_second"])', function(e) {
        e.stopPropagation();
    ...
    // 習得チェックボックスの処理
    $(document).on('change', 'input[type="checkbox"][name*="_second"]', function(e) {
        e.stopPropagation();
    ...
    // 初期化時にアクティブタブを設定
    if ($('.progress-tab.active').length === 0 && $('.progress-tab').length > 0) {
        $('.progress-tab').first().addClass('active');
        var firstSubject = $('.progress-tab').first().data('subject');
        $('.subject-progress[data-subject="' + firstSubject + '"]').show();
    }
    
    // デバッグ情報
    console.log('進捗管理タブのJavaScript初期化完了');
    console.log('タブ数: ' + $('.progress-tab').length);
    console.log('アクティブタブ: ' + $('.progress-tab.active').data('subject'));
});
</script>
...
@media (max-width: 782px) {
    ...
    .color-settings-panel div[style*="display: flex"] {
        flex-direction: column !important;
        gap: 10px !important;
    }
...
.subject-progress[style*="display: none"] {
    display: none !important;
}

.subject-progress[style*="display: block"] {
