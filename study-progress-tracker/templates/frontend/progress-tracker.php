            ...
            <div class="chapters-container">
                <?php foreach ($chapter_structure[$subject_key]['chapters'] as $chapter_id => $chapter_data): 
                    ...
                    if (isset($progress_data[$subject_key]['chapters'][$chapter_id])) {
                        ...
                        $total_sections = isset($chapter_data['section_data']) ? count($chapter_data['section_data']) : $chapter_data['sections'];
                        
                        if (count($chapter_sections) == $total_sections) {
                            $chapter_completed = true;
                            $chapter_class .= ' completed';
                            
                            // 習得チェック
                            $mastered_count = 0;
                            foreach ($chapter_sections as $section_data) {
                                if (is_array($section_data) && isset($section_data['items'])) {
                                    // 新形式：全項目が習得レベルかチェック
                                    $all_mastered = true;
                                    $section_info = $chapter_data['section_data'][$section_id] ?? null;
