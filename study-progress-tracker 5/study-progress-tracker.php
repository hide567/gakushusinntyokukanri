...
class StudyProgressTracker {
    ...
    public function enqueue_scripts() {
        ...
        $localize_data = array(
            ...
            'first_check_color' => isset($settings['first_check_color']) ? $settings['first_check_color'] : '#e6f7e6',
            'second_check_color' => isset($settings['second_check_color']) ? $settings['second_check_color'] : '#ffebcc',
    ...
    public function render_countdown_shortcode($atts) {
        ...
        $settings = get_option('spt_settings', array());
        $exam_date = !empty($atts['date']) ? $atts['date'] : (isset($settings['exam_date']) ? $settings['exam_date'] : '');
        $exam_title = !empty($atts['title']) ? $atts['title'] : (isset($settings['exam_title']) ? $settings['exam_title'] : '試験');
    ...
    public function ajax_toggle_progress() {
        if (!wp_verify_nonce(isset($_POST['nonce']) ? $_POST['nonce'] : '', 'spt_frontend')) {
            wp_send_json_error(array('message' => 'セキュリティ検証に失敗しました'));
            return;
        }
        
        $subject = sanitize_key(isset($_POST['subject']) ? $_POST['subject'] : '');
        $chapter = intval(isset($_POST['chapter']) ? $_POST['chapter'] : 0);
        $section = intval(isset($_POST['section']) ? $_POST['section'] : 0);
        $item = intval(isset($_POST['item']) ? $_POST['item'] : 0);
        $level = intval(isset($_POST['level']) ? $_POST['level'] : 0);
    ...
    public function ajax_add_chapter() {
        if (!wp_verify_nonce(isset($_POST['nonce']) ? $_POST['nonce'] : '', 'spt_admin_ajax')) {
            wp_send_json_error();
            return;
        }
        
        $subject = sanitize_key(isset($_POST['subject']) ? $_POST['subject'] : '');
        $chapter_name = sanitize_text_field(isset($_POST['chapter_name']) ? $_POST['chapter_name'] : '');
    ...
    public function ajax_add_section() {
        ...
        $subject = sanitize_key(isset($_POST['subject']) ? $_POST['subject'] : '');
        $chapter = intval(isset($_POST['chapter']) ? $_POST['chapter'] : 0);
        $section_name = sanitize_text_field(isset($_POST['section_name']) ? $_POST['section_name'] : '');
    ...
    public function ajax_add_item() {
        ...
        $chapter = intval(isset($_POST['chapter']) ? $_POST['chapter'] : 0);
        $section = intval(isset($_POST['section']) ? $_POST['section'] : 0);
        $item_name = sanitize_text_field(isset($_POST['item_name']) ? $_POST['item_name'] : '');
