...
<div class="spt-progress-tracker spt-flexible <?php echo esc_attr($atts['style'] ?? 'default'); ?>" 
        ...
        <div class="spt-subject-content" style="display: none;">
            ...
            <div class="spt-chapters">
                <?php foreach ($subject_structure['chapters'] as $chapter_id => $chapter_data): 
                    $chapter_progress = $subject_progress[$chapter_id] ?? array();
                    ...
                    if (!empty($chapter_data['sections'])) {
                        foreach ($chapter_data['sections'] as $section_id => $section_data) {
                            if (!empty($section_data['items'])) {
                                ...
                                if (isset($chapter_progress[$section_id])) {
                    ...
                    <div class="spt-chapter-header">
                        <div class="spt-chapter-title-container">
                            <div class="spt-chapter-top">
                                ...
                                <span class="spt-chapter-title"><?php echo esc_html($chapter_data['name'] ?? '第' . $chapter_id . '章'); ?></span>
                    ...
                    <div class="spt-chapter-content" style="display: none;">
                        <?php if (!empty($chapter_data['sections'])): ?>
                            <?php foreach ($chapter_data['sections'] as $section_id => $section_data): 
                                $section_progress = $chapter_progress[$section_id] ?? array();
                                $section_total = !empty($section_data['items']) ? count($section_data['items']) : 0;
                            ...
                            <div class="<?php echo esc_attr($section_class); ?>" data-section="<?php echo esc_attr($section_id); ?>">
                                <div class="spt-section-header">
                                    <div class="spt-section-title-container">
                                        <span class="spt-section-title"><?php echo esc_html($section_data['name'] ?? '節' . $section_id); ?></span>
                                ...
                                <?php if ($interactive && !empty($section_data['items'])): ?>
                                <div class="spt-items">
                                    <?php foreach ($section_data['items'] as $item_id => $item_name): 
                                        $item_level = intval($section_progress[$item_id] ?? 0);
                                        $item_class = 'spt-item';
                                        if ($item_level >= 1) {
                                ...
                                <?php elseif (!empty($section_data['items'])): ?>
