...
// サンプルデータの場合
if (!$map_data && isset($this->sample_data[$atts['data']])) {
...
if (!$map_data) {
    $error_message = 'マップデータが見つかりませんでした。データ属性: ' . esc_html($atts['data']);
}
?>
...
<div class="mindmap-container <?php echo $atts['search'] === 'true' || $atts['details'] === 'true' ? 'mindmap-phase2' : ''; ?>" 
    ...
    <?php if ($error_message): ?>
        <div class="mindmap-error" style="padding: 20px; text-align: center; color: #666; border: 1px solid #ddd; border-radius: 5px;">
            ...
            <small>利用可能なデータ: <?php echo esc_html(implode(', ', array_keys($this->sample_data ?: array()))); ?></small>
        </div>
    <?php else: ?>
        ...
        (function() {
            ...
            if (document.readyState === 'loading') {
                document.addEventListener('DOMContentLoaded', function() {
                    initializeMindMap('<?php echo esc_js($unique_id); ?>');
                });
            } else {
                initializeMindMap('<?php echo esc_js($unique_id); ?>');
            }
            
            function initializeMindMap(containerId) {
                ...
                if (typeof window.MindMapRenderer !== 'undefined') {
                    try {
                        ...
                        if (container) {
                            ...
                            console.log('マインドマップが正常に初期化されました:', containerId);
                        } else {
                            console.error('マインドマップコンテナが見つかりません:', containerId);
                        }
                    } catch (error) {
                    ...
                    console.warn('MindMapRendererクラスが読み込まれていません。JavaScriptファイルの読み込みを確認してください。');
                    ...
                    setTimeout(function() {
                        ...
                        if (container) {
                            const canvas = container.querySelector('.mindmap-canvas');
                            if (canvas && canvas.innerHTML.includes('読み込み中')) {
                                canvas.innerHTML = '<div style="text-align: center; padding: 50px; color: #999;">JavaScript読み込みエラー<br><small>ページを再読み込みしてください</small></div>';
                            }
                        }
                    }, 3000);
                }
